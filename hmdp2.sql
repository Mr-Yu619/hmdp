...
CREATE TABLE `tb_blog_comments` (
                                    ...
                                    `reply_to_comment_id` bigint(20) UNSIGNED NOT NULL DEFAULT 0 COMMENT '回复的目标评论id',
                                    `content` text NOT NULL COMMENT '回复的内容',
                                    `liked` int(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '点赞数',
                                    `status` tinyint(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '状态，0：正常，1：被举报，2：禁止查看',
                                    `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                    `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
                                    PRIMARY KEY (`id`) USING BTREE,
                                    KEY `idx_blog_parent` (`blog_id`, `parent_id`) COMMENT '按博客和父评论查询索引'
...
CREATE TABLE `tb_follow`(
    ...
    PRIMARY KEY (`id`) USING BTREE,
    KEY `idx_user` (`user_id`) COMMENT '查询关注列表'
)ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci ROW_FORMAT = Compact;


-- 这是秒杀券开始做活动的表，会有库存，开始时间结束时间限制了
...
CREATE TABLE `tb_shop_type` (
                                ...
                                `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
                                PRIMARY KEY (`id`),
                                UNIQUE KEY `uk_name` (`name`) COMMENT '类型名称唯一索引'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  ROW_FORMAT=Dynamic
  COMMENT='商铺类型表';
...
-- 用到了分表设计，tb_user存储的是高频访问的核心数据，比如手机号，密码，用于登陆，鉴权
-- tb_user_info 存储的是低频访问的辅助数据，比如城市，性别，生日等
...
-- 前面还有一个优惠卷id和库存以及生效时间等等的信息，也是分表
